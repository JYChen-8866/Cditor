...
async fn sqlite_newer_document_schema_loads_session_in_readonly_mode() {
    ...
    let loaded = load_session_from_storage(Arc::new(storage), 42, 1, options)
        .await
        .unwrap();
...
async fn sqlite_cold_start_replays_durable_emergency_log_into_dirty_session() {
    ...
    let loaded = load_session_from_storage(storage.clone(), 42, 1, options.clone())
        .await
        .unwrap();
    ...
    let reopened = load_session_from_storage(storage.clone(), 42, 1, options)
        .await
        .unwrap();
...
async fn injected_storage_loads_session_through_backend_neutral_host() {
    ...
    let options = Cditor::new()
        ...
        .into_options();
    let loaded = load_session_from_options(&options).await.unwrap().unwrap();
    let opened = loaded.prepared.into_opened();
    assert_eq!(
        opened.session.snapshot().unwrap().document_id,
        runtime_document_id
    );
    assert_eq!(opened.report.payloads_loaded, 1);
    assert_eq!(
        opened.session.persistence_snapshot().unwrap().backend,
        Some(StorageBackendKind::Postgres)
    );
}
