...
pub(crate) fn is_supported_autolink_target(target: &str) -> bool {
    ...
    // Vendored from velotype: gpui's `http_client::Uri` (a URL-style URI
    // parser) is replaced with a lightweight scheme extractor.
    uri_scheme(target).is_some_and(|scheme| matches!(scheme, "http" | "https"))
}

/// Extracts the URI scheme when `target` is `scheme://...` shaped.
fn uri_scheme(target: &str) -> Option<&str> {
    ...
    Some(&target[..scheme_len])
}

/// Normalizes a link/image reference label the way velotype's image module
/// does (lowercased, whitespace collapsed). Kept local so the vendored link
/// parser does not depend on the image module.
