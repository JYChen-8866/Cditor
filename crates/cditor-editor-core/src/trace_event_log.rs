...
impl TraceEvent {
    ...
    pub fn async_result_discarded(
        frame_id: u64,
        timestamp_ms: u64,
        event: AsyncResultDiscardedEvent,
    ) -> Self {
        Self {
            frame_id,
            timestamp_ms,
            kind: TraceEventKind::AsyncResultDiscarded,
            payload: TraceEventPayload::AsyncResultDiscarded(event),
        }
    }
}
...
mod tests {
    ...
    fn old_request_discard_simulation_is_observable() {
        ...
        log.push(TraceEvent::async_result_discarded(
            2,
            21,
            AsyncResultDiscardedEvent {
                block_id: Some(88),
                request_generation: 3,
                current_generation: 5,
                request_content_version: Some(10),
                current_content_version: Some(12),
                reason: "content version mismatch".to_owned(),
            },
        ));
