...
use cditor_text::{
    FontFaceKey, FontInstanceKey, FontSynthesisKey, ParleyFontSlant, ParleyLayoutOptions,
    ParleyLineHeight, ParleyPaintFont, ParleyTextStyleConfig, TextLayoutInput, TextLayoutSurfaceId,
    TextTheme, build_parley_layout, register_font_data,
};
...
fn variable_coordinates_change_exact_raster_pixels() {
    ...
    let thin = paint_font_fixture("'wght' 100", ParleyFontSlant::Normal);
    let black = paint_font_fixture("'wght' 900", ParleyFontSlant::Normal);
    assert_ne!(
...
fn faux_skew_changes_exact_raster_pixels() {
    clear_exact_raster_cache();
    register_fixture_font();
    let normal = paint_font_fixture("'wght' 450", ParleyFontSlant::Normal);
    let italic = paint_font_fixture("'wght' 450", ParleyFontSlant::Italic);
    assert!(
...
fn repeated_exact_raster_uses_bounded_cache() {
    clear_exact_raster_cache();
    register_fixture_font();
    let fixture = paint_font_fixture("'wght' 450", ParleyFontSlant::Normal);
...
fn paint_font_fixture(
    variations: &str,
    slant: ParleyFontSlant,
) -> (ParleyPaintFont, u32, f32, u32) {
    ...
    let layout = build_parley_layout(
        ...
        &ParleyLayoutOptions {
            ...
            base_style: ParleyTextStyleConfig {
                ...
                font_slant: slant,
                line_height: ParleyLineHeight::Absolute(40.0),
...
fn raster_fixture(fixture: &(ParleyPaintFont, u32, f32, u32)) -> Vec<u8> {
    let (value, _) = cached_or_rasterize(raster_key(fixture), raster_source(fixture)).unwrap();
    cache_value_bytes(&value)
}

fn raster_key(fixture: &(ParleyPaintFont, u32, f32, u32)) -> ExactRasterKey {
    ExactRasterKey {
...
fn raster_source(fixture: &(ParleyPaintFont, u32, f32, u32)) -> RasterFontSource<'_> {
    RasterFontSource {
        data: fixture.0.data(),
