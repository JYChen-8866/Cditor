...
pub fn detect_delimiter_at_caret(text: &str, caret: usize) -> Option<DelimiterPairDetection> {
    ...
    if caret >= 2
        && text.get(caret.saturating_sub(2)..caret) == Some("**")
        && let Some(range) = find_matching_delimiter_pair(text, caret, "**")
    {
    ...
    if caret >= 1 && text.as_bytes()[caret - 1] == b'*' {
        ...
        if !is_double_star && let Some(range) = find_matching_delimiter_pair(text, caret, "*") {
            return Some(DelimiterPairDetection {
                source_range: range,
                delimiter: "*".to_string(),
