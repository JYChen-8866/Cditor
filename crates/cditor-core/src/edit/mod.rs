...
pub use origin::ChangeOrigin;
pub use selection::{
    AccessibilitySelectionProjection, BlockSelectionFragment, DocumentSelection,
    InnerSelectionAnchor, NormalizedSelection, SelectionEndpoint, SelectionRange,
    SelectionResolveError, TextAffinity, TextPosition, UnifiedDocumentSelection,
};
...
pub use transaction_codec::{
    TransactionDecodeError, TransactionDecodeOutcome, decode_transaction, encode_transaction,
...
pub use undo::{
    ExternalUndoBlobRef, NonUndoableEditEvent, UndoExternalizationCandidate,
    UndoExternalizationJob, UndoGroupBoundary, UndoGroupingPolicy, UndoPayload, UndoStack,
    UndoStep,
};
...
mod tests {
    ...
    #[test]
    fn large_undo_snapshot_preserves_transaction_until_externalized_and_hydrates() {
        let mut undo = UndoStack::new(UndoGroupingPolicy {
            typing_merge_window_ms: 1_000,
            inline_block_snapshot_limit: 1,
        });
        let transaction = EditTransaction::paste_blocks(
            77,
            10,
            0,
            vec![
        ...
        assert_eq!(candidate.snapshot_id, 1);
        assert_eq!(candidate.block_count, 2);
        assert_eq!(candidate.transaction, &transaction);

        let reference = ExternalUndoBlobRef {
            snapshot_id: 1,
            storage_key: "sqlite:undo:1".to_owned(),
            checksum: "sha256:test".to_owned(),
            encoded_len: 123,
            block_count: 2,
        };
        assert!(undo.mark_externalized(reference.clone()));
        ...
        assert_eq!(undo.next_undo_external_blob(), Some(&reference));
        let step = undo.take_undo_step().unwrap();
        undo.commit_undo_step(step);
        assert_eq!(undo.next_redo_external_blob(), Some(&reference));
        let step = undo.take_redo_step().unwrap();
        undo.commit_redo_step(step);

        assert!(undo.hydrate_externalized(1, transaction.clone()));
        let candidate = undo.pending_externalization().unwrap();
        assert_eq!(candidate.transaction, &transaction);
    }

    #[test]
    fn moving_undo_steps_between_stacks_does_not_clone_large_transaction() {
        let mut undo = UndoStack::new(UndoGroupingPolicy {
            typing_merge_window_ms: 1_000,
            inline_block_snapshot_limit: 1,
        });
        ...
        undo.record_transaction(transaction);
        let transaction_address =
            undo.pending_externalization().unwrap().transaction as *const EditTransaction;

        assert!(undo.pop_undo().is_some());
    ...
    #[test]
    fn externalization_job_abort_restores_the_owned_transaction() {
        let mut undo = UndoStack::new(UndoGroupingPolicy {
            typing_merge_window_ms: 1_000,
            inline_block_snapshot_limit: 1,
        });
        ...
        undo.record_transaction(transaction.clone());
        let job = undo.begin_externalization().unwrap();
        assert!(undo.pending_externalization().is_none());
        assert!(undo.abort_externalization(job));
        assert_eq!(
    ...
    fn inverse_transaction_restores_selection_and_anchor_once() {
        ...
        let mut step = UndoStep {
            payload: UndoPayload::InlineSmall(Box::new(tx)),
            boundary: None,
