use super::*;
use cditor_core::rich_text::{TableCellPayload, TableColumnPayload, TableRowPayload};

fn table_grid(rows: usize, columns: usize) -> cditor_core::rich_text::TablePayload {
    cditor_core::rich_text::TablePayload {
        rows: (0..rows)
            .map(|_| TableRowPayload {
                cells: (0..columns).map(|_| TableCellPayload::default()).collect(),
                ..TableRowPayload::default()
            })
            .collect(),
        columns: (0..columns)
            .map(|_| TableColumnPayload::default())
            .collect(),
        ..cditor_core::rich_text::TablePayload::default()
    }
}
...
async fn postgres_integration_table_structure_survives_save_and_reopen() {
    use cditor_core::rich_text::{BlockPayload, InlineSpan};
...
async fn postgres_integration_table_track_sizes_survive_save_and_reopen() {
    use cditor_core::rich_text::{BlockPayload, TableTrackSize};
...
async fn postgres_integration_table_merge_and_align_survive_save_and_reopen() {
    use cditor_core::rich_text::{BlockPayload, InlineSpan, TableCellAlign, TableCellMerge};
