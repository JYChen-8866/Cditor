...
pub(crate) enum DbEditOperation {
    ...
    Collection {
        op: CollectionEditOperation,
    },
    Comment {
        op: CommentEditOperation,
    },
    Asset {
        op: AssetEditOperation,
    },
}
...
impl From<&EditTransaction> for DbEditTransaction {
    fn from(tx: &EditTransaction) -> Self {
        Self {
            id: tx.id,
            origin: tx.origin,
            preconditions: tx.preconditions.clone(),
            ...
            before_selected_blocks: tx.before_selected_blocks.clone(),
            after_selected_blocks: tx.after_selected_blocks.clone(),
    ...
    fn from(op: &EditOperation) -> Self {
        match op {
            ...
            EditOperation::Table(op) => Self::Table {
                op: Box::new(DbTableEditOperation::from(op)),
            },
            EditOperation::Text(op) => Self::Text { op: op.clone() },
            EditOperation::Block(op) => Self::Block { op: op.clone() },
            EditOperation::Collection(op) => Self::Collection { op: op.clone() },
            EditOperation::Comment(op) => Self::Comment { op: op.clone() },
            EditOperation::Asset(op) => Self::Asset { op: op.clone() },
        }
    }
}
