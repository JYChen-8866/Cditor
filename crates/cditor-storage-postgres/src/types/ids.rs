...
pub(crate) type PgDocumentId = Uuid;
pub(crate) type PgBlockId = Uuid;
...
pub(crate) fn pg_document_id_from_runtime(id: DocumentId) -> PgDocumentId {
    Uuid::from_u128(DOCUMENT_ID_NAMESPACE | id as u128)
}

pub(crate) fn pg_block_id_from_runtime(id: BlockId) -> PgBlockId {
    Uuid::from_u128(BLOCK_ID_NAMESPACE | id as u128)
}

pub(crate) fn runtime_document_id_from_pg(id: PgDocumentId) -> Option<DocumentId> {
    let raw = id.as_u128();
    ((raw & NAMESPACE_MASK) == DOCUMENT_ID_NAMESPACE).then_some((raw & LOW_64_BITS) as u64)
}

pub(crate) fn runtime_block_id_from_pg(id: PgBlockId) -> Option<BlockId> {
    let raw = id.as_u128();
