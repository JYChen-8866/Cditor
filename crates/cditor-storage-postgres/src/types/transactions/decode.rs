...
impl From<DbEditTransaction> for EditTransaction {
    fn from(tx: DbEditTransaction) -> Self {
        Self {
            ...
            ops: std::sync::Arc::new(tx.ops.into_iter().map(EditOperation::from).collect()),
            inverse_ops: std::sync::Arc::new(
                tx.inverse_ops
                    .into_iter()
            ...
            before_selected_blocks: tx.before_selected_blocks,
            after_selected_blocks: tx.after_selected_blocks,
...
impl From<DbEditOperation> for EditOperation {
    fn from(op: DbEditOperation) -> Self {
        match op {
            ...
            DbEditOperation::Text { op } => Self::Text(op),
            DbEditOperation::Block { op } => Self::Block(op),
            DbEditOperation::Collection { op } => Self::Collection(op),
            DbEditOperation::Comment { op } => Self::Comment(op),
            DbEditOperation::Asset { op } => Self::Asset(op),
        }
    }
}
