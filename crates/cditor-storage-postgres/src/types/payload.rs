...
pub enum DbBlockPayload {
    ...
    Collection {
        ...
        schema_version: u32,
        #[serde(default)]
        properties: Vec<cditor_core::rich_text::CollectionPropertyPayload>,
        #[serde(default)]
        views: Vec<cditor_core::rich_text::CollectionViewPayload>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl TryFrom<&BlockPayload> for DbBlockPayload {
    ...
    fn try_from(payload: &BlockPayload) -> Result<Self, Self::Error> {
        Ok(match payload {
            ...
            BlockPayload::Columns(columns) => Self::Columns {
                columns: columns.columns.clone(),
                gap_milli: columns.gap_milli,
            },
            ...
            BlockPayload::Collection(collection) => Self::Collection {
                collection_id: collection.collection_id,
                title: collection.title.clone(),
                schema_version: collection.schema_version,
                properties: collection.properties.clone(),
                views: collection.views.clone(),
                active_view_id: collection.active_view_id,
            },
...
impl From<DbBlockPayload> for BlockPayload {
    fn from(payload: DbBlockPayload) -> Self {
        match payload {
            ...
            DbBlockPayload::Columns { columns, gap_milli } => {
                Self::Columns(ColumnsGroupPayload { columns, gap_milli })
            }
            ...
            DbBlockPayload::Collection {
                ...
                views,
                active_view_id,
            } => Self::Collection(CollectionPayload {
                collection_id,
                title,
                schema_version,
                properties,
