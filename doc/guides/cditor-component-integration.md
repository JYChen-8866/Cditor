...
| `cditor-desktop` | Demo/OpenAI/远程图片的默认桌面组装 | 是 |
...
- 持久化由宿主实现并注入 `DocumentStorage`；组件不打开文件或数据库。
...
`cditor_sdk::Cditor` 是不可变 builder 风格的配置对象。它不持有 View 或 Runtime；使用
持久化文档时只持有宿主注入的内容端口，不持有路径、URL、连接池或 migration 配置。

### 5.1 数据来源

| 方法 | 行为 |
| --- | --- |
| `Cditor::new()` / `.memory()` | 创建空白内存文档 |
| `.demo()` | 使用小型演示文档 |
| `.large_demo()` | 使用 100,000 Block 性能文档 |
| `.with_document_id(id).with_storage(storage)` | 注入文档 ID 和已打开的 `Arc<dyn DocumentStorage>` |

数据库 adapter 属于宿主应用，不属于 Cditor workspace：

```rust
use cditor_sdk::Cditor;

let storage = app_infrastructure.open_document_store().await?;
let editor = Cditor::new()
    .with_document_id(42)
    .with_storage(storage);
```

SDK 和 desktop 本身不依赖 SQLite、PostgreSQL、SQLx 或数据库环境变量。
...
持久化内容端口必须同时指定 `document_id`。无效配置由 `build_component` 返回
...
1. 验证 document target；
2. 接收宿主已经打开的 `DocumentStorage`；
3. 创建 `EditorSession` 并保持 Runtime 单一串行所有者；
4. 注入 AI、远程图片和平台能力；
5. 构造实现 `CditorViewContract` 的 GPUI View。

不要在 SDK、Core、Runtime、Session 或 GPUI 组件内读取数据库环境变量、创建数据库 pool、
执行 migration 或管理 workspace/knowledgebase。数据库装配属于外层应用。
