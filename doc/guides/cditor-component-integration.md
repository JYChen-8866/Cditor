...
PostgreSQL 后端必须指定 `document_id`。推荐的 `.build(cx)` 在缺少文档 ID 时返回 `CditorError::InvalidInput`；兼容入口 `.build_view(cx)` 仍构造 `LoadFailed` View。
...
| `.with_payload_window_size(usize)` | 设置 PostgreSQL payload 分片加载窗口，最小为 1 |
...
- `crates/app/src/api/cold_start.rs`：PostgreSQL 冷启动高级接口。
