...
use crate::core::rich_text::ImagePayload;
...
const IMAGE_FOOTER_HEIGHT_PX: f32 = 40.0;
const IMAGE_FOOTER_WITH_CAPTION_HEIGHT_PX: f32 = 60.0;
...
pub fn render_image_block(
    ...
    let caption = image_caption(image);
    ...
    if let Some((_, height)) = display_size {
        let measured_height = image_block_measured_height(height, !caption.is_empty());
    ...
    let card = card
        .child(if let Some(render_image) = loaded {
            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .child(image_display_name(image)),
                )
                .when(!caption.is_empty(), |this| {
                    this.child(div().text_color(rgb(theme.muted)).child(caption))
                }),
        );
...
fn render_image_resize_handle(
    ...
    div()
        ...
        .bg(rgb(theme.surface))
        .opacity(0.82)
        .hover(|s| s.border_color(rgb(theme.focused)).opacity(1.0))
...
fn image_block_measured_height(image_height_px: f32, has_caption: bool) -> f64 {
    let footer = if has_caption {
        IMAGE_FOOTER_WITH_CAPTION_HEIGHT_PX
    } else {
        IMAGE_FOOTER_HEIGHT_PX
    };
    f64::from(image_height_px + footer + IMAGE_BLOCK_OUTER_CHROME_HEIGHT_PX)
}
...
fn image_display_name(image: &ImagePayload) -> String {
    if !image.alt.trim().is_empty() {
        return image.alt.trim().to_owned();
    }
    image
        .source
        .rsplit(['/', '\\'])
...
fn image_caption(image: &ImagePayload) -> String {
    image.caption.trim().to_owned()
}
...
mod tests {
    ...
    #[test]
    fn image_footer_uses_alt_as_title_and_caption_as_subtitle() {
        ...
        assert_eq!(image_caption(&image), "caption");
        assert_eq!(image_display_name(&image), "alt");
    }

    #[test]
    fn image_block_measured_height_uses_displayed_image_height() {
        assert_eq!(
            image_block_measured_height(512.0, false),
            f64::from(512.0 + IMAGE_FOOTER_HEIGHT_PX + IMAGE_BLOCK_OUTER_CHROME_HEIGHT_PX)
        );
        assert_eq!(
            image_block_measured_height(512.0, true),
            f64::from(
                512.0 + IMAGE_FOOTER_WITH_CAPTION_HEIGHT_PX + IMAGE_BLOCK_OUTER_CHROME_HEIGHT_PX
            )
        );
    }
